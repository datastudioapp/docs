...
<Aside type="tip">
  ...
  [helloworld-datakit](https://github.com/datastudioapp/helloworld-datakit)
  repository.
</Aside>
...
The datakit CLI needs Docker to run. Install Docker following the instructions
for your operating system [here](https://www.docker.com/get-started/).

### Installing the CLI

Before we begin, we need to install the
[datastudio-cli](https://pypi.org/project/datastudio-cli/) tool.
...
```bash
git clone https://github.com/datastudioapp/python-run-base
cd python-run-base
./build.sh
```
...
Let's create a new datakit. The `ds` CLI tool provides a convenient command to
do this:
...
```json title="helloworld/algorithm.json" ""name": "x"" ""name": "result""
{
  ...
  "code": "algorithm.py",
  "container": "datastudio/python-run-base:latest",
  "signature": {
    "inputs": [
      {
        "name": "x",
...
```bash
ds reset  # This deletes any existing runs
ds init
```
