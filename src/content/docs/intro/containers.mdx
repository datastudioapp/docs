...
By default, algorithms in a datakit are executed in the
[`python-run-base`](https://github.com/datastudioapp/python-run-base) Docker
container, which provides commonly used libraries such as `numpy` and `pandas`.
If you need a library not included in this default container, you can easily
extend it and create a custom Docker container to run your algorithm.
...
FROM datastudioapp/python-run-base:latest
...
```bash
ds reset  # Reset to ensure we clear any old run configurations
ds init
ds load data data/tabulardata.csv  # Our algorithm will fail if we forget to load some input data
ds run
```

If everything is set up correctly, your algorithm should print the version of
`scikit-learn` that you just installed in your custom container:

```bash
=> Executing helloworld.run
╭─────────────────────
│ 1.8.0                                                                                                                                                                                                             │
╰─────────────────────
=> Executed helloworld.run successfully
```

Congrats - you've successfully executed your algorithm in a custom environment.
