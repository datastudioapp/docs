...
```bash
ds reset
ds init  # Ensure we initialise a new run to use the updated algorithm signature
ds set function "mode"
```
...
```bash
ds reset  # Reset the datakit to a clean state again
ds init helloworld.median
```
...
```bash
ds load data data/tabulardata.csv  # Load our data
ds set function "median"           # Set our calculation function to "median"
ds run                             # Run the algorithm
```
...
```bash
ds init helloworld.mean
```

Now we can load our input data, set the function we want to use to calculate
the result, and run the datakit again:

```bash
ds load data data/tabulardata.csv
ds set function "mean"
ds run
```
...
```bash
ds set-run helloworld.median
ds show result  # Print the "median" run result
```
